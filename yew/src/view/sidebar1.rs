...
pub fn sidebar1(ctx: &Context<BoltApp>, active: u8) -> Html {
    html! {
        <div class="sidebar1">
            <div class={if active == 0 {"sidebaritem sidebaritem-selected pointer"} else {"sidebaritem pointer"} } onclick={ctx.link().callback(|_| Msg::SwitchPage(Page::Home))}>
            ...
            <div class={if active == 1 {"sidebaritem sidebaritem-selected pointer"} else {"sidebaritem pointer"} } onclick={ctx.link().callback(|_| Msg::SwitchPage(Page::Collections) )}>
